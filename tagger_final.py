...
Preposition, Conjuction and Interjection. Based on the word that is present and its context based on 
...
Step 1 : Accept the sys aruguments which are the train , test and output file name 
...
Step 5: In the tagged word list creare a new tag called NN for the class unidentified.
...
Step 9: If the token in the train has been associated with more than one POS tag then  pick the POS tag with the highest occurence
...
if __name__ == "__main__":
    ...
    if final_test is not None and tagged_tokens is not None:
        ...
        for test_token in test_tokens:
            matched_pos_final = 'NN'  # if word in  not existed in training (Unknow word) use NN
